...
const people = {
  asgard:{name:'Asgard',id:'1187421394695376968',img:'assets/asgard.webp',bio:'Creative direction, visual identity and studio quality.'},
  diego:{name:'Diego',id:'1248575202946846755',img:'assets/diego.webp',bio:'Technical direction, gameplay systems and scripting.'},
  kzyh:{name:'Kzyh',id:'1360958504063471747',img:'assets/kzyh.webp',bio:'Studio operations and official game server infrastructure.'},
  espanhola:{name:'ESPANHOLA',id:'1023336106839777391',img:'assets/espanhola.jpg',bio:'Studio coordination, operations and community organization.'},
  guga:{name:'Guga',id:'738811406501281844',img:'assets/guga.webp',bio:'Technical systems that power Solara experiences.'},
  shinzaki:{name:'Shinzaki',id:'1475261480495611956',img:'assets/shinzaki.webp',bio:'Clear interfaces and intuitive player experiences.'},
  sanny:{name:'Sanny',id:'1537203659580907631',img:'assets/sanny.webp',bio:'Visual effects for abilities, combat and key moments.'},
  susu:{name:'Susu',id:'1181353153082433638',img:'assets/susu.webp',bio:'3D assets that shape the look of each experience.'},
  osnofa:{name:'Osnofa',id:'1413268553620000950',img:'assets/osnofa.webp',bio:'Worlds and environments built for atmosphere and gameplay.'},
  gojo:{name:'Gojo',id:'925220475519504404',img:'assets/gojo.webp',bio:'Visual effects that make gameplay feel impactful.'},
  igortyy:{name:'igortyy',id:'595417031587921921',img:'assets/igortyy.webp',bio:'Gameplay logic, systems and interactive mechanics.'},
  meg:{name:'Meg',id:'867405277569613894',img:'assets/meg.webp',bio:'Character and gameplay animation.'}
};
...
function memberCard(key,role,{featured=false}={}){
  ...
  return `<article class="member-card ${featured?'featured-member':''} reveal">
    <div class="member-media"><img src="${p.img}" alt="${p.name}" loading="lazy" decoding="async"><span class="role-chip">${role}</span></div>
    <div class="member-body"><div class="member-head"><h3>${p.name}</h3></div><p>${p.bio}</p>
      <div class="member-actions">
        <a href="https://discord.com/users/${p.id}" target="_blank" rel="noreferrer" aria-label="Open ${p.name} on Discord">${icon('i-discord')}<span>Discord</span></a>
        <button class="copy-id" type="button" data-copy="${p.id}" aria-label="Copy ${p.name}'s Discord ID">${icon('i-copy')}<span>Copy ID</span></button>
      </div>
    </div>
  </article>`;
}
...
function home(){
  ...
  <section class="section connect"><div class="shell"><article class="connect-card reveal"><span class="kicker">SOLARA COMMUNITY</span><h2>FOLLOW WHAT<br><span class="gradient">WE BUILD NEXT.</span></h2><p>Updates, sneak peeks and future testing opportunities.</p>${button('Join Discord',DISCORD,'primary','i-discord')}</article></div></section>`;
}

function games(){
  return `<section class="directory view-enter"><div class="shell"><header class="directory-head"><span class="kicker">GAMES</span><h1>OUR <span class="gradient">WORLDS.</span></h1><p>Experiences currently in development at Solara Inc.</p></header>${gameCard()}</div></section>`;
}

function team(){
  return `<section class="directory team-directory view-enter"><div class="shell"><header class="directory-head"><span class="kicker">STUDIO DIRECTORY</span><h1>THE PEOPLE<br>BEHIND <span class="gradient">SOLARA.</span></h1><p>Development, design, art and studio operations.</p></header>${teamSections.map(section=>`<section class="team-section"><header class="team-title reveal"><h2>${section.title}</h2><p>${section.desc}</p></header><div class="team-grid ${section.members.length===1?'single':''}">${section.members.map(([key,role])=>memberCard(key,role)).join('')}</div></section>`).join('')}</div></section>`;
}
...
document.addEventListener('click',async e=>{const c=e.target.closest('.copy-id');if(c){const original=c.innerHTML;try{await navigator.clipboard.writeText(c.dataset.copy);c.innerHTML=icon('i-copy')+'<span>Copied</span>'}catch{c.innerHTML=icon('i-copy')+'<span>Failed</span>'}setTimeout(()=>c.innerHTML=original,1000)}if(e.target.closest('#mobileNav a'))document.body.classList.remove('menu-open')});
